...
station = 'UAK24_A1' #'ECOSENS_HF22_D1'
irradiance_height = -0.5 #120000#
detector_separation = 0.37
first_wavelength = 350e-9#380e-9#
last_wavelength = 850e-9 #1020e-9 #
n_wavelengths = 640
band_width = 10e-9
use_sat = False
use_satellite_wavelengths = use_sat
use_satellite_viewing_angles = use_sat
use_satellite_response_functions = use_sat
use_satellite_time_point = use_sat

if use_satellite_wavelengths:
    toa_meta = flick.toa_meta('input/'+station+'/'+station+'_toa_meta.txt')

ocean_meta = flick.ocean_meta('input/'+station+'/'+station+'_meta.txt')
...
aerosols = 0.4 #at 440 measured in dennis 2015: 0.0-0.4 (sim says 550 nm)
wv = 5 #5.2-36.6 m measured in dennis 2015 times 1307 conversion factor from precip. to vapor
clouds = 1e-4 

def radiation(f, time_point, detector_height, wavelengths):
    f.set('aerosol_od', aerosols)
    f.set('cloud_liquid', clouds)
    f.set('ozone', 0.004)
    f.set('pressure', 1000e2)
    f.set('temperature', 273+16)
    f.set('water_vapor',wv)
    f.set('mp_names', 'input/'+station+'/'+station)
    f.set('mp_concentrations', ocean_meta.spm*1)
    f.set('mp_scattering_scaling_factors', 1)    
    f.set('mcdom_names', 'input/'+station+'/'+station)
...
def get_wavelengths(radiation_type):
    to_m = 1e-9
    if use_satellite_wavelengths:
        wl = flick.table('input/'+station+'/'+station+'_toa_radiance.txt')[:,0]*to_m
    elif radiation_type == 'irradiance':
        wl = flick.table('input/'+station+'/'+station+'_ocean_irradiance.txt')[:,0]*to_m
    elif radiation_type == 'radiance':
        wl = flick.table('input/'+station+'/'+station+'_ocean_radiance.txt')[:,0]*to_m
...
print('aerosols: ' + str(aerosols))
print('watervapor: ' + str(wv))
